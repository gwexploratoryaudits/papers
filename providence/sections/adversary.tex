...
\end{lemma}
\begin{proof}
 Let $[n_1, \ldots, n_j]$ be a round schedule, and lets assume that an EoR \BRAVO audit stopped exactly at
 round $j$, 
 after observing $k_1, \ldots, k_j$ ballots for the announced winner in each round respectively. This means that
 the following conditions hold:
 \begin{enumerate}
  \item for all $i = 1, \ldots, j-1$: 
%   $k_j < k^{p_a, p_0, \alpha}_{min, \BRAVO, n_j}$,
  $\frac{P[{K_i = k_{i}} | H_a, n_i]}{P[K_i = {k_{i}} | H_0, n_i]} < \frac{1}{\alpha}$,
  \item for $i = j$: % $k_r \geq k^{p_a, p_0, \alpha}_{min, \BRAVO, n_r}.$
  $\frac{P[{K_j = k_{j}} | H_a, n_j]}{P[K_j = {k_{j}} | H_0, n_j]} \geq \frac{1}{\alpha}$,
 \end{enumerate}
