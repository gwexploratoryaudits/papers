...
\caption{The real time as estimated by Equation~\ref{eq:real_time} for varying $p$ with expected values as estimated by our simulations. Error bars show the $0.25$ and $0.75$ quantiles. Unlike Figures \ref{fig:avg_bals}, \ref{fig:avg_bals_ratio}, and \ref{fig:with_round_workload}, the quantiles stil differ for large $p$ because the first round size is no longer a constant; the number of ballots drawn in the first round in Fairfax County is variable.}
\label{fig:real_time}
\end{figure}
